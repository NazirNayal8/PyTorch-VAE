...
parser.add_argument("--feature_reduction", type=str,
                    default="mean", choices=["mean", "concat", "pe"])
...
def extract_tsne_with_kmeans(id_dataset, ood_datasets, output_folder):
    ...
    if args.feature_reduction == "mean":
        quantized_id = quantized_id.view(N, C, H * W).mean(dim=2)
        quantized_ood = [dataset.outputs.quantized.view(
            N, C, H * W).mean(dim=2) for dataset in ood_datasets]
    elif args.feature_reduction == "concat":
        quantized_id = quantized_id.view(N, C * H * W)
        quantized_ood = [dataset.outputs.quantized.view(
            N, C * H * W) for dataset in ood_datasets]
    elif args.feature_reduction == "pe":
        pe = positionalencoding2d(C, H, W).unsqueeze(0).repeat(N, 1, 1, 1)
        quantized_id = (quantized_id + pe).view(N, C, H * W).mean(dim=2)
        quantized_ood = [(dataset.outputs.quantized + pe).view(
            N, C, H * W).mean(dim=2) for dataset in ood_datasets]
    ...
    for i in range(len(ood_datasets) + 1):
        ax["clusters"].scatter(tsne_rep[clusters == i, 0], tsne_rep[clusters ==
                               i, 1], color=get_color(), label=f'C{i+1}', alpha=alpha)
    ...
    for i, d in enumerate([id_dataset] + ood_datasets):
        ...
        label += f"({d.name})"
        ax["dataset"].scatter(tsne_rep[last:current_idx, 0],
                              tsne_rep[last:current_idx, 1], color=get_color(), label=label, alpha=alpha)
...
def get_kth_codebook_distances(dataset, k):
    ...
    kth_distance = dataset.outputs.distances.kthvalue(
        dim=2, k=k).values.mean(dim=1)
...
def extract_codebook_distances(k, id_dataset, ood_datasets, output_folder):
    ...
    if isinstance(k, int):
        ...
        raise ValueError(
            f"K should be a number or a list idiot. Focus!!, given was {type(k)}")
    ...
    for kth in k:
        ...
        ood_distances = [get_kth_codebook_distances(
            dataset, kth) for dataset in ood_datasets]
        ...
        plt.savefig(os.path.join(output_folder,
                    f"codebook_distance_to_{kth}.png"))


def plot_perplexity_density(id_dataset, ood_datasets, output_folder):

    id_perplexity = id_dataset.outputs.perplexity
    ood_perplexity = [dataset.outputs.perplexity for dataset in ood_datasets]

    df = pd.DataFrame()
    df["value"] = torch.cat([id_perplexity] + ood_perplexity, dim=0)
    label = [f"ID ({id_dataset.name})"] * id_perplexity.shape[0]
    for i, dataset in enumerate(ood_datasets):
        label += [f"OoD ({dataset.name})"] * ood_perplexity[i].shape[0]
    ...
    plt.savefig(os.path.join(output_folder, "perplexity_density.png"))
...
def main():
    ...
    for model_name in model_names:
        ...
        try:
            ...
            print(f"Loading model {model_name} failed. Skipping")
            continue
