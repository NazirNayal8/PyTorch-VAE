...
class GPT(nn.Module):
    ...
    start_token_mode supports the following options:
    - after_last: add a new token with index vocab_size as the start of sequence token
    - label: shift all tokens with the number of classes and append the class number to beginning
            of the sequence
    - one_hot: to be implemented

    """
    def __init__(self, vocab_size, block_size, n_layer, n_head, n_embd,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, start_token_mode='after_last', num_classes=10):
        super().__init__()
        config = GPTConfig(vocab_size=vocab_size, block_size=block_size,
                           embd_pdrop=embd_pdrop, resid_pdrop=resid_pdrop, attn_pdrop=attn_pdrop,
                           n_layer=n_layer, n_head=n_head, n_embd=n_embd,
                           n_unmasked=n_unmasked)
        if start_token_mode == 'after_last':
            extra_token = 1
        elif start_token_mode == 'label':
            extra_token = num_classes
        else:
            extra_token = 0
        # input embedding stem
        self.tok_emb = nn.Embedding(config.vocab_size + extra_token, config.n_embd)
        ...
        self.config = config
        self.start_token_mode = start_token_mode
    ...
    def forward(self, idx, embeddings=None, targets=None, label=0, flattened_input=False):
        ...
        if self.training:
            if self.start_token_mode == 'after_last':
                starter_token = torch.ones((idx.shape[0], 1), dtype=torch.long, device=idx.device) * self.config.vocab_size
                idx = torch.cat((starter_token, idx), dim=1)[:,:-1]
            elif self.start_token_mode == 'label':
                starter_token = label.unsqueeze(1)
                idx = torch.cat((starter_token, idx + self.num_classes), dim=1)[:,:-1]
        ...
        if not flattened_input:
            ...
            logits = logits.view(B, self.config.vocab_size, H, W) # (B, C, H, W)

        return logits
    
    def generate(self, label=0, shape=(8,8), temperature=1.0, top_k=None, batch_size=1):
        
        block_size = self.get_block_size()
        self.eval()
        steps = np.prod(shape)
        if self.start_token_mode == 'after_last':
            x = torch.ones((batch_size, 1), dtype=torch.long, device='cuda') * self.config.vocab_size
        elif self.start_token_mode == 'label':
            x = label.unsqueeze(1)
        else:
            x = torch.zeros((batch_size, 1), dtype=torch.long, device='cuda')

        for k in range(steps):
            x_cond = x if x.size(1) <= block_size else x[:, -block_size:]  # crop context if needed
            logits = self(x_cond, flattened_input=True, label=label)
            ...
            if self.start_token_mode == "label":
                ix = ix + self.num_classes
            x = torch.cat((x, ix), dim=1)
        
        if self.start_token_mode == "label":
            x = x - self.num_classes
        x = x[:, 1:].view(batch_size, *shape).contiguous()
        return x
